...
DATA_LIST = []
path_train = 'usual_data/train.txt'
with open(path_train, 'r', encoding='UTF-8') as f:
    data_list = json.load(f)
    for line in data_list:
        content = line["content"]
        label_list = ['angry', 'surprise', 'fear', 'happy', 'sad', 'neutral']
        label = label_list.index(line['label'])
        DATA_LIST.append((content, to_categorical(label, 6)))
DATA_LIST = np.array(DATA_LIST)

DATA_LIST_TEST = []
path_test = 'usual_data/test.txt'
with open(path_test, 'r', encoding='UTF-8') as f:
    data_list = json.load(f)
    for line in data_list:
        content = line["content"]
        DATA_LIST_TEST.append((content, to_categorical(0, 6)))
...
def run_cv(nfold, data, data_labels, data_test):
    ...
    train_model_pred = np.zeros((len(data), 6))
    test_model_pred = np.zeros((len(data_test), 6))
    ...
    for i, (train_fold, test_fold) in enumerate(kf):
        ...
        checkpoint = ModelCheckpoint('./bert_dump/' + str(i) + '.hdf5', monitor='val_accuracy',verbose=2, save_best_only=True, mode='max', save_weights_only=True) #保存最好的模型
 
        train_D = data_generator(X_train, shuffle=True)
        valid_D = data_generator(X_valid, shuffle=True)
        test_D = data_generator(data_test, batch_size=8,shuffle=False)
        #模型训练
        model.fit_generator(
            train_D.__iter__(),
