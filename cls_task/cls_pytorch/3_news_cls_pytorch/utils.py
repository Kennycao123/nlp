...
def validate(model, dataloader):
    ...
    with torch.no_grad():
        for (batch_seqs, batch_seq_masks, batch_seq_segments, batch_labels) in dataloader:
            ...
            # all_prob.extend(torch.max(probabilities.data,1)[1].cpu().numpy())
    ...
    # _multiclass_roc_auc_score(all_labels, all_prob, labels=None, multi_class='ovo', average="macro", sample_weight=None)
...
def predict(model, dataloader, device):
    ...
    id = [0, 1, 2]
    # item = ['民生','文化','娱乐','体育','财经','房产','汽车','教育','科技','军事','旅游 ','国际','证券',
    #         '农业','电竞']
    item = ['正面','中性','负面']
    item_dict = dict(zip(id,item))
    # device = model.device
    with torch.no_grad():
        result = []
        for (batch_seqs, batch_seq_masks, batch_seq_segments, batch_labels) in dataloader:
            ...
            result.append(item_dict[int(torch.argmax(probabilities))])
