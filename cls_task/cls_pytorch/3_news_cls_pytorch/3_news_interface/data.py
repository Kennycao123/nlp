...
class DataPrecessForSentence(Dataset):
    ...
    对文本进行处理
    """
    def __init__(self, bert_tokenizer, LCQMC_file, pred=None,max_char_len = 100):
        """
        bert_tokenizer :分词器
        LCQMC_file     :语料文件
        """
    ...
    def get_input(self, file,pred=None):
        ...
        if pred:
            '''
            预测接口
            file：[a, b] list，各元素为需要判断的句子的list列表
            '''
            sentences = []
            for i, j in enumerate(file):
                sentences.append(j)
            sentences = map(HanziConv.toSimplified, sentences)
            labels = [0] * len(file)
        else:
            df = pd.read_csv(file)
            # sentences = map(HanziConv.toSimplified, df['text'].values)
    ...
    def trunate_and_pad(self, tokens_seq):
        ...
        """
        # 对超长序列进行截断
        if len(tokens_seq) > ((self.max_seq_len - 3)//2):
            tokens_seq = tokens_seq[0:(self.max_seq_len - 3)//2]
        # 分别在首尾拼接特殊符号
